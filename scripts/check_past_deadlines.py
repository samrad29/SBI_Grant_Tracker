...
def _fetch_grants(conn, *, limit: int | None, status: str | None) -> list[dict]:
...
def main() -> int:
    ...
    try:
        grants = _fetch_grants(conn, limit=args.limit, status=args.status)
    finally:
        conn.close()
    ...
    for i, grant in enumerate(grants, start=1):
        ...
        if verdict.deadline_passed:
            ...
            print(f"  -> PASSED ({verdict.confidence})")
        else:
